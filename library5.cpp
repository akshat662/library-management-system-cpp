...
public:
    ...
    void searchFuzzyTitle() {
        string query;
        cin.ignore();
        cout << "Enter book title (approx): "; getline(cin, query);
        vector< pair<int, int> > matches;
        for (size_t i = 0; i < books.size(); ++i) {
            int dist = levenshtein(books[i].title, query);
            if (dist <= 5) matches.push_back(make_pair(dist, (int)i));
        }
        sort(matches.begin(), matches.end());
        for (size_t i = 0; i < matches.size(); ++i)
            books[matches[i].second].display();
        if (matches.empty()) cout << "No similar titles found.\n";
    }
    ...
    void rateBook() {
        int id, rating;
        string comment;
        cout << "Enter Book ID to rate: "; cin >> id;
        cin.ignore();
